...
public class GTInternal {
	...
	private boolean isCanTryConnect() {
	    return (curConnState == CONNECT_STATE_NOT_CONNECTED ||
	...
	public boolean connect(Context hostContext, AbsGTParaLoader loader) {
		if (hostContext == null || !isCanTryConnect()) {
		    return false;
        }
	...
	private void startConnectGT() {
        ...
        gtIntent.setComponent(new ComponentName(
                GTInternal.GT_PACKAGE_NAME, GTInternal.SERVICE_NAME));
	...
	public boolean isCanTryDisconnect() {
	    return (curConnState == CONNECT_STATE_CONNECTED);
	}
	...
	public void disconnect(Context hostContext) {
		if (isCanTryDisconnect()) {
			...
			if (disconnectGT(hostContext.getPackageName())) {
	...
	public boolean disconnectGT(String cur_pkgName) {
	    return (null != gtCom && gtCom.disconnectGT(cur_pkgName));
	}
	...
	//=======================================关于输出参数=======================================================
	public void setOutPara(String paraName, Object value, boolean isGlobal, String unit) {
		if (null == paraName || null == value) {
			return;
		}

		if (null != curConnState) {
			curConnState.setOutPara(paraName, value, isGlobal, unit);
		}
	}
