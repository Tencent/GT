...
public class ConnectedState extends AbsDataCachedConnState {
	...
	@Override
	public void setOutPara(String paraName, Object value, Object ...extras) {
		// 这个状态不走缓存，直接入队
		dataCacheController.setOutPara(paraName, value, extras);
	}
	...
	private InPara getInPara(String key, boolean isGlobal) {
		IService iService = GTInternal.getInstance().getGTService();
		try {
			return isGlobal
                    ? iService.getGlobalInPara(key)
					: iService.getInPara(key);
