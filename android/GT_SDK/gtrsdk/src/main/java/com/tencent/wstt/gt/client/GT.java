...
public class GT {
	...
	public static void setOutPara(String paraName, Object value, Object...extras) {
		...
		if (extras != null) {
		    ...
		    if (idx < extras.length && extras[idx] instanceof Boolean) {
		        isGlobal = (boolean) extras[idx++];
            }

            if (idx < extras.length && extras[idx] instanceof String) {
		        unit = (String)extras[idx++];
            }
        }

        GTInternal.INSTANCE.setOutPara(paraName, value.toString(), isGlobal, unit);
	}
	 ...
	 * java6编译器不需要此方法
	 */
	public static void setOutPara(String paraName, Object value, String unit) {
		if (!ENABLE) {
			return;
		}

		if (null != value) {
			GTInternal.INSTANCE.setOutPara(paraName, value.toString(), false, unit);
		}
	}
