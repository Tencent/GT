...
public class PerfTaskConsumer {
	...
	public PerfTaskConsumer(final IService gtService,
			final DataCacheController dataCacheController)
	{
		thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					while (flag) {
						AidlTask task = dataCacheController.takePerfTask();
						if (null != task) {
							if (task instanceof PerfDigitalEntry)
							{
								// 发送到控制台处理
								gtService.setPerfDigitalEntry((PerfDigitalEntry)task);
							}
						}
		...
		}, PerfTaskConsumer.this.getClass().getSimpleName());
	}
	
	public void start()
	{
		flag = true;
		thread.setPriority(Thread.MIN_PRIORITY + 1); // 比日志优先级高点
		thread.start();
	}
