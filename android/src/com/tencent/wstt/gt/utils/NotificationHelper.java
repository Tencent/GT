...
public class NotificationHelper {
	...
	static public Notification notify(Context c, int notifyId, int iconResId,
			...
			Class<?> cls, int flag, int notify_way) {
		final Resources res = ((Activity) c).getResources();

		return notify(c, notifyId, iconResId, res.getString(textResId),
				soundResId, res.getString(titleResId),
				res.getString(contentResId), cls, flag, notify_way);
	}
	...
	static public Notification notify(Context c, int notifyId, int iconResId,
			...
			String contentText, Class<?> cls, int flag, int notify_way) {

		Notification n = genNotification(c, notifyId, iconResId,
				notifyShowText, soundResId, titleText, contentText, cls, flag,
				notify_way);
	...
	static public Notification genNotification(Context c, int notifyId,
			int iconResId, int textResId, int soundResId, int titleResId,
			int contentResId, Class<?> cls, int flag, int notify_way) {
		final Resources res = ((Activity) c).getResources();
		// Uri.parse("android.resource://" + GTApp.getContext().getPackageName()
		// + "/" +R.raw.greattit);
		return genNotification(c, notifyId, iconResId,
				res.getString(textResId), soundResId,
	...
	static public Notification genNotification(Context c, int notifyId,
		...
		n.flags = flag;
		// 通知方式设置
		n.defaults = notify_way;

		// 设置通知参数
		n.setLatestEventInfo(c, titleText, contentText, ip);

		if (soundResId == 0) {
			n.sound = Uri.parse(GTApp.getContext().getFilesDir().getPath()
					+ "/" + "greattit.mp3");
